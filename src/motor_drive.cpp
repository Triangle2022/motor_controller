...
void MotorDrive<Elmo>::setPDOMapping()
{
  ...
  for (uint8_t i = 1; i <= ec_slavecount; i++) {
...

    {
      ...
      ec_SDOwrite(i, rxPDOMapping, 3, false, sizeof(val), &val, EC_TIMEOUTRXM);
...
void MotorDrive<Elmo>::setTorque(uint8_t motorID, int16_t torque)
{
...
bool MotorDrive<Elmo>::setDesiredMotorTorque(uint8_t motorID, double desiredMotorTorque)
{
  ...
  double motorTorque = compensateMotorTorque(desiredMotorTorque);
  ...
  setTorque(motorID, static_cast<int16_t>(motorTorque));
